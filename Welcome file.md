...
> **Current Configuration**: Local authentication routes are **disabled**. 
> The H5P server uses a **shared JWT secret key** from the LMS for token verification.
> For full details, see [AUTHENTICATION.md](./AUTHENTICATION.md).

### Token Verification Flow (Active)

This is the **only active authentication flow**. The LMS generates JWT tokens using a shared secret key, and the H5P server verifies them.
...
sequenceDiagram
    ...
    WebPortal->>H5PServer: POST /auth/verify<br/>Header: Bearer {token}
    H5PServer->>H5PServer: Extract token
    H5PServer->>SessionCache: Check cache
    
    alt Cache Hit
        SessionCache-->>H5PServer: Cached user data
        H5PServer-->>WebPortal: 200 {valid: true, user}
    else Cache Miss
        SessionCache-->>H5PServer: null
        H5PServer->>H5PServer: Verify JWT signature<br/>(shared secret key)
        
        opt External API Verification
            H5PServer->>ExternalAPI: GET /auth/verify<br/>Header: Bearer {token}
            ExternalAPI-->>H5PServer: User data + permissions
        end
        
        H5PServer->>SessionCache: Cache user session (TTL: 5min)
        H5PServer-->>WebPortal: 200 {valid: true, user}
    end
...
Both the LMS and H5P server must use the **same secret key**:
